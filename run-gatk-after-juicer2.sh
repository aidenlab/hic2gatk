...
if [ "$first_stage" == "sort" ]; then
	...
	if [[ -z ${fraction} ]]; then
		...
		if [[ ${target_coverage} -ne 0 ]]; then
		{
			...
			samtools view -u -d "rt:0" -d "rt:1" -d "rt:2" -d "rt:3" -d "rt:4" -d "rt:5" -d "rt:7" -@ $((threads * 2)) -F 0x400 -q $mapq -s 0.001 $bam | samtools sort -@ $threads -m 6G -o test.sorted.bam
	...
	samtools view -u -d "rt:0" -d "rt:1" -d "rt:2" -d "rt:3" -d "rt:4" -d "rt:5" -d "rt:7" -@ $((threads * 2)) -F 0x400 -q $mapq -s $fraction $bam |  samtools sort -@ $threads -m 6G -o reads.sorted.bam
