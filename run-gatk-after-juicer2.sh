...
if [ "$first_stage" == "sort" ]; then
	...
	if [[ -z ${fraction} ]]; then
		...
		if [[ ${target_coverage} -ne 0 ]]; then
		{
			...
			samtools cat -@ $((threads * 2)) $bam | samtools view -u -d "rt:0" -d "rt:1" -d "rt:2" -d "rt:3" -d "rt:4" -d "rt:5" -d "rt:7" -@ $((threads * 2)) -F 0x400 -q $mapq -s 0.001 - | samtools sort -@ $threads -m 6G -o test.sorted.bam
	...
	# sort and index	
	samtools cat -@ $((threads * 2)) -h mega_header.bam $bam | samtools view -u -d "rt:0" -d "rt:1" -d "rt:2" -d "rt:3" -d "rt:4" -d "rt:5" -d "rt:7" -@ $((threads * 2)) -F 0x400 -q $mapq -s $fraction - |  samtools sort -@ $threads -m 6G -o reads.sorted.bam
...
if [ "$first_stage" == "recalibrate_variants" ]; then
	...
	if [[ ! -z $indelRecalibrationArg ]]; then
		...
		gatk --java-options "-Xmx4G -XX:+UseParallelGC -XX:ParallelGCThreads=40" VariantRecalibrator -V ${vcf} -O out.recal -mode INDEL --tranches-file out.tranches -tranche 100.0 -tranche 99.9 -tranche 99.0 -tranche 90.0 -an QD -an FS -an MQRankSum -an ReadPosRankSum -an SOR --max-gaussians 4 ${indelRecalibrationArg}
		...
		gatk --java-options "-Xmx4G -XX:+UseParallelGC -XX:ParallelGCThreads=20" ApplyVQSR -V ${vcf} -O out.vcf --recal-file out.recal --tranches-file out.tranches -truth-sensitivity-filter-level 99.0 -mode INDEL --create-output-variant-index
